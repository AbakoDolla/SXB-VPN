...
function CredentialsModal({ credentials, onClose }: { credentials: GeneratedCreds; onClose: () => void }) {
  ...
  const copyAll = () => navigator.clipboard.writeText(
    t('operations.settings.credentialsCopy', { url: 'https://vpnsxb.afrihall.com', name: credentials.name, email: credentials.email, password: credentials.password, role: credentials.role })
  );
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm">
      <div className="w-full max-w-md p-6 bg-[#0a0d14] border border-[#1a2535] rounded-2xl shadow-2xl">
        <div className="flex items-center gap-3 mb-5">
          ...
          <div>
            <h2 className="text-base font-bold text-white">{t('operations.settings.credentialsTitle', { role: credentials.role })}</h2>
            <p className="text-xs text-gray-400">{t("operations.settings.shareCredentials")}</p>
          </div>
        </div>
        <div className="mb-4 p-3 rounded-lg bg-amber-500/10 border border-amber-500/20 text-xs text-amber-300">
          {t("operations.settings.passwordWarning")}</div>
        <div className="space-y-3">
          {[
            { label: t("operations.common.email"), value: credentials.email, copied: copiedEmail, setter: setCopiedEmail },
            { label: t("operations.common.password"), value: credentials.password, copied: copiedPass, setter: setCopiedPass },
            ...
            <div key={label}>
              ...
              <div className="flex gap-2">
                ...
                <button aria-label={t(copied ? "operations.common.copied" : "operations.common.copy")} onClick={() => copy(value, setter)} className="px-3 py-2 rounded-lg bg-[#0f1218] border border-[#1a1f2e] text-gray-400 hover:text-white cursor-pointer shrink-0">
          ...
          <div>
            <label className="block text-xs font-semibold text-gray-500 uppercase tracking-wider mb-1">{t("operations.settings.dashboardUrl")}</label>
        ...
        <div className="mt-5 flex gap-2">
          <button onClick={copyAll} className="flex-1 py-2 text-xs font-semibold rounded-xl bg-[#0f1218] border border-[#1a1f2e] text-gray-300 hover:text-white cursor-pointer flex items-center justify-center gap-1.5">
            <Copy size={12} /> {t("operations.settings.copyAll")}</button>
          <button onClick={onClose} className="flex-1 py-2 text-xs font-semibold rounded-xl bg-cyan-500 text-black hover:bg-cyan-400 cursor-pointer">{t("operations.common.close")}</button>
        </div>
      </div>
    </div>
  );
...
const TABS = [
  { id: "profile", label: "operations.settings.tab.profile", icon: User },
  { id: "team", label: "operations.settings.tab.team", icon: Users },
  { id: "security", label: "operations.settings.tab.security", icon: Shield },
  { id: "api", label: "operations.settings.tab.api", icon: Key },
  { id: "language", label: "operations.common.language", icon: Globe },
];
...
export default function SettingsView({ currentUser, onUserUpdated, onNavigate }: SettingsViewProps) {
  ...
  const [profileError, setProfileError] = useState<{ cause: unknown } | null>(null);
  ...
  const handleAvatarUpload = async () => {
    ...
    try {
      ...
      if (!res.ok) throw new Error("operations.settings.avatarUploadError");
      const data = await res.json();
      setAvatarUrl(data.avatarUrl);
      setAvatarPreview(null);
      if (onUserUpdated && data.user) onUserUpdated(data.user);
    } catch (err) {
      alert(errorMessage(err, "operations.settings.avatarUploadError"));
    } finally {
  ...
  const handleGenerateToken = async () => {
    ...
    try {
      ...
      alert(errorMessage(err, "operations.settings.tokenGenerateError"));
    } finally {
      setTokenGen(false);
    }
  };
  ...
  const handleRevokeToken = async (id: string) => {
    if (!confirm(t("operations.settings.revokeConfirm"))) return;
    try {
      await revokeAdminToken(id);
      await loadAdminTokens();
    } catch (err) { alert(errorMessage(err, "operations.sessions.revokeError")); }
  };
    ...
    <div className="space-y-5 max-w-3xl">
      ...
      <div>
        <div className="flex items-center gap-2 mb-1">
          ...
          <h1 className="text-xl font-bold text-white">{t("operations.settings.title")}</h1>
        </div>
        <p className="text-xs text-gray-500">{t("operations.settings.description")}</p>
      </div>
      ...
      {activeTab === "profile" && (
        <div className="space-y-4">
          <div className={sectionClass}>
            <h3 className="text-sm font-semibold text-white mb-4 flex items-center gap-2"><User className="w-4 h-4 text-cyan-400" />{t("operations.settings.personalInfo")}</h3>
            {/* Avatar */}
              ...
              <div className="relative">
                <div className="w-14 h-14 rounded-xl bg-[#0f1218] border border-[#1a1f2e] overflow-hidden flex items-center justify-center">
                  {(avatarPreview || avatarUrl) ? (
                    <img src={avatarPreview || avatarUrl!} alt={t("operations.settings.avatar")} className="w-full h-full object-cover" />
                  ) : (
                ...
                {avatarPreview ? (
                  <div className="flex gap-2 mt-2">
                    <button onClick={handleAvatarUpload} disabled={avatarUploading} className="text-xs px-3 py-1.5 bg-cyan-500/20 text-cyan-400 border border-cyan-500/30 rounded-lg hover:bg-cyan-500/30 cursor-pointer flex items-center gap-1 disabled:opacity-50">
                      ...
                      {t("operations.common.save")}</button>
                    <button aria-label={t("operations.common.cancel")} onClick={cancelAvatarPreview} className="text-xs px-3 py-1.5 bg-[#0f1218] border border-[#1a1f2e] rounded-lg text-gray-400 hover:text-white cursor-pointer">
                      <X className="w-3 h-3" />
                    </button>
                  </div>
                ) : (
                  <button onClick={() => fileInputRef.current?.click()} className="mt-2 text-xs text-cyan-400 hover:text-cyan-300 cursor-pointer flex items-center gap-1">
                    <Camera className="w-3 h-3" /> {t("operations.settings.changePhoto")}</button>
                )}
            ...
            <form onSubmit={handleProfileSave} className="space-y-4">
              <div>
                <label className={labelClass}>{t("operations.settings.fullName")}</label>
                <input value={profileName} onChange={e => setProfileName(e.target.value)} type="text" className={inputClass} placeholder={t("operations.settings.namePlaceholder")} />
              </div>
              <div>
                <label className={labelClass}>{t("operations.common.email")}</label>
                <input value={currentUser?.email || ""} type="email" className={inputClass + " opacity-60 cursor-not-allowed"} disabled />
              </div>
              <div>
                <label className={labelClass}>{t("operations.settings.phone")}</label>
              ...
              {profileError && <p className="text-xs text-red-400 bg-red-500/10 border border-red-500/20 rounded-lg px-3 py-2">{errorMessage(profileError.cause, "operations.settings.profileSaveError")}</p>}
              <button type="submit" disabled={profileSaving} className="flex items-center gap-2 px-4 py-2 text-xs font-semibold rounded-lg bg-cyan-500 hover:bg-cyan-400 text-black transition-all disabled:opacity-60 cursor-pointer">
                ...
                {profileSaved ? t("operations.settings.saved") : t("operations.common.save")}
              </button>
            </form>
          </div>
        </div>
      ...
      {activeTab === "team" && (
        <div className={sectionClass}>
          <h3 className="text-sm font-semibold text-white mb-2 flex items-center gap-2">
            <Users className="w-4 h-4 text-cyan-400" />{t("operations.settings.teamManagement")}</h3>
          <p className="text-xs text-gray-400 mb-4">
            {t("operations.settings.teamIntro")}{" "}<span className="text-gray-200"> {t("operations.settings.accountsPath")}</span>{t("operations.settings.teamDetails")}</p>
          <button
            type="button"
          ...
          >
            ...
            {t("operations.settings.openAccounts")}</button>
        </div>
      )}
      ...
      {activeTab === "security" && (
        <div className={sectionClass}>
          <h3 className="text-sm font-semibold text-white mb-4 flex items-center gap-2"><Lock className="w-4 h-4 text-cyan-400" />{t("operations.settings.accountSecurity")}</h3>
          <div className="space-y-3">
            {[
              { label: t("operations.settings.jwt"), desc: t("operations.settings.jwtDescription"), status: t("operations.settings.securityActive"), color: "text-emerald-400 bg-emerald-500/10 border-emerald-500/20" },
              { label: t("operations.settings.refreshToken"), desc: t("operations.settings.refreshDescription"), status: t("operations.settings.securityActive"), color: "text-emerald-400 bg-emerald-500/10 border-emerald-500/20" },
              { label: t("operations.settings.rbac"), desc: t("operations.settings.rbacDescription"), status: t("operations.settings.securityActive"), color: "text-emerald-400 bg-emerald-500/10 border-emerald-500/20" },
              { label: t("operations.settings.encryption"), desc: t("operations.settings.encryptionDescription"), status: t("operations.settings.securityActive"), color: "text-emerald-400 bg-emerald-500/10 border-emerald-500/20" },
            ].map(item => (
          ...
          <div className="mt-4 p-3 bg-amber-500/10 border border-amber-500/20 rounded-xl text-xs text-amber-400">
            {t("operations.settings.passwordHelp")}</div>
        </div>
      )}
        ...
        <div className="space-y-4">
          <div className={sectionClass}>
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-sm font-semibold text-white flex items-center gap-2"><Key className="w-4 h-4 text-cyan-400" />{t("operations.settings.adminTokens")}</h3>
              <button
              ...
              >
                ...
                {t("operations.settings.generateToken")}</button>
            </div>
            {tokenLoading ? (
              ...
              <div className="py-8 text-center text-gray-600">
                ...
                <p className="text-sm">{t("operations.settings.noTokens")}</p>
              </div>
            ) : (
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex items-center gap-3 mt-1">
                        <span className={`text-[10px] font-medium ${token.status === 'active' ? 'text-emerald-400' : 'text-gray-500'}`}>
                          ● {token.status === 'active' ? t("operations.common.active") : t("operations.common.revoked")}
                        </span>
                        {token.usedAt && <span className="text-[10px] text-gray-600">{t('operations.settings.usedAt', { date: formatDate(token.usedAt) })}</span>}
                        <span className="text-[10px] text-gray-600">{t('operations.settings.createdAt', { date: formatDate(token.createdAt) })}</span>
                      </div>
                    </div>
                    <button aria-label={t(copiedToken === token.token ? "operations.common.copied" : "operations.common.copy")} onClick={() => copyToken(token.token)} className="p-1.5 rounded-lg text-gray-500 hover:text-white hover:bg-white/5 cursor-pointer">
                      {copiedToken === token.token ? <CheckCheck className="w-3.5 h-3.5 text-emerald-400" /> : <Copy className="w-3.5 h-3.5" />}
                    </button>
                    {token.status === 'active' && (
                      <button aria-label={t("operations.sessions.revoke")} onClick={() => handleRevokeToken(token.id)} className="p-1.5 rounded-lg text-gray-500 hover:text-rose-400 hover:bg-rose-500/10 cursor-pointer">
            ...
            {[{ code: 'fr', label: t('core.languages.fr'), flag: '🇫🇷' }, { code: 'en', label: t('core.languages.en'), flag: '🇬🇧' }].map(lang => (
              ...
              >
                ...
                <div>
                  ...
                  {language === lang.code && <p className="text-[10px] text-cyan-400">{t("operations.common.active")}</p>}
                </div>
              </button>
            ))}
          </div>
