...
const createStatusConfig = (t: ReturnType<typeof useTranslation>['t']) => ({
  active:  { label: t('commerce.common.active'),   cls: 'text-emerald-400 bg-emerald-500/10' },
  used:    { label: t('commerce.common.used'), cls: 'text-blue-400 bg-blue-500/10' },
  expired: { label: t('commerce.common.expired'),  cls: 'text-amber-400 bg-amber-500/10' },
  revoked: { label: t('commerce.common.revoked'), cls: 'text-rose-400 bg-rose-500/10' },
});

export default function TokensView({ currentUserRole }: TokensViewProps) {
  const { t, locale, formatBytes, errorMessage } = useTranslation();
  ...
  const handleCreate = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!clientId) { setFormError('commerce.tokens.selectClientError'); return; }
    setFormError('');
    setSubmitting(true);
    try {
  ...
  const handleRevoke = async (id: string) => {
    if (isSupport) return;
    if (!confirm(t('commerce.tokens.confirmRevoke'))) return;
    try {
      await revokeToken(id);
      await Promise.all([load(), refreshAccess()]);
    } catch (err: any) {
      alert(errorMessage(err, 'commerce.common.errorRevoke'));
    }
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold tracking-tight text-white">
            {t('commerce.tokens.title')}
          </h1>
          <p className="text-sm text-gray-400 mt-1">
            {t('commerce.tokens.subtitleBefore')}<code className="text-cyan-400 text-xs">SXB-XXXX-XXXX-XXXX</code>{t('commerce.tokens.subtitleAfter')}
          </p>
        </div>
        {!isSupport && (
          <button
      ...
      <div className="relative w-full md:w-80">
        ...
        <input
          type="text"
          placeholder={t('commerce.tokens.search')}
          value={search}
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-[#1a1f2e]">
                  <th className="text-left px-5 py-3 text-xs font-semibold text-gray-400 uppercase tracking-wider">{t('commerce.common.tokenTechnical')}</th>
                  <th className="text-left px-5 py-3 text-xs font-semibold text-gray-400 uppercase tracking-wider">{t('commerce.common.client')}</th>
                  <th className="text-left px-5 py-3 text-xs font-semibold text-gray-400 uppercase tracking-wider">{t('commerce.common.quota')}</th>
                  <th className="text-left px-5 py-3 text-xs font-semibold text-gray-400 uppercase tracking-wider">{t('commerce.common.expiration')}</th>
                  <th className="text-left px-5 py-3 text-xs font-semibold text-gray-400 uppercase tracking-wider">{t('commerce.common.status')}</th>
                  {!isSupport && (
                    <th className="text-right px-5 py-3 text-xs font-semibold text-gray-400 uppercase tracking-wider">{t('commerce.common.actions')}</th>
                  )}
                </tr>
              </thead>
              <tbody className="divide-y divide-[#1a1f2e]">
                {filtered.length === 0 && (
                  <tr>
                    <td colSpan={6} className="text-center py-16">
                      ...
                      <p className="text-gray-500">{t('commerce.tokens.empty')}</p>
                      {canCreate && (
                        <button
                          onClick={() => setShowModal(true)}
                          className="mt-3 text-cyan-400 hover:text-cyan-300 text-sm"
                        >
                          {t('commerce.tokens.first')}
                        </button>
                      )}
                    </td>
                  </tr>
                ...
                {filtered.map((tok) => {
                  ...
                  const clientName = client?.user?.name || t('commerce.tokens.unknownClient');
                  ...
                  return (
                    <tr key={tok.id} className="hover:bg-white/[0.02] transition-colors">
                      ...
                      <td className="px-5 py-3.5 text-gray-300 text-sm font-mono">
                        {formatBytes(tok.quota)}
                      </td>
                      <td className="px-5 py-3.5 text-gray-400 text-xs">
                        {tok.expiration
                          ? new Date(tok.expiration).toLocaleDateString(locale)
                          : '—'}
                      </td>
                      <td className="px-5 py-3.5">
          ...
          <div className="w-full max-w-md bg-[#0d111b] border border-[#1a1f2e] rounded-2xl shadow-2xl">
            <div className="flex items-center justify-between px-6 py-4 border-b border-[#1a1f2e]">
              <div className="flex items-center gap-2">
                ...
                <h2 className="text-base font-semibold text-white">{t('commerce.tokens.generate')}</h2>
              </div>
            ...
            <form onSubmit={handleCreate} className="px-6 py-5 space-y-4">
              {!!formError && (
                <div className="p-3 bg-rose-500/10 border border-rose-500/30 rounded-lg text-rose-400 text-sm">
                  {errorMessage(formError, 'commerce.common.errorCreate')}
                </div>
              )}

              <div>
                <label className="block text-xs font-semibold text-gray-400 mb-1.5 uppercase tracking-wider">
                  {t('commerce.common.vpnClientRequired')}
                </label>
                <div className="relative">
                  ...
                  >
                    <option value="">{t('commerce.common.chooseClientDots')}</option>
                    {clients.map(c => (
                ...
                {clients.length === 0 && (
                  <p className="text-xs text-amber-400 mt-1">{t('commerce.tokens.noClients')}</p>
                )}
              </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  <label className="block text-xs font-semibold text-gray-400 mb-1.5 uppercase tracking-wider">
                    {t('commerce.common.durationDays')}
                  </label>
                  <input
