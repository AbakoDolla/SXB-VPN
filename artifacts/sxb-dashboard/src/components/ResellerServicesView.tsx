  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold tracking-tight text-white flex items-center gap-2">
            ...
            {t('commerce.services.title')}
          </h1>
          <p className="text-gray-400 text-sm mt-1">
            {t('commerce.services.subtitle')}
          </p>
        </div>
        <button
          onClick={load}
      ...
      <div className="flex items-start gap-2 p-3 bg-violet-500/5 border border-violet-500/20 rounded-xl text-xs text-violet-200">
        ...
        <p>
          {t('commerce.services.privacy')}
        </p>
      </div>

      {!!error && (
        <div className="p-3 bg-rose-500/10 border border-rose-500/20 rounded-xl text-rose-400 text-sm">{errorMessage(error, 'commerce.services.loadError')}</div>
      )}
      ...
      {loading ? (
        ...
        <div className="text-center py-16 bg-[#0f1218] border border-[#1a1f2e] rounded-xl">
          ...
          <p className="text-gray-400 text-sm">{t('commerce.services.empty')}</p>
          <p className="text-gray-600 text-xs mt-1">
            {t('commerce.services.emptyHint')}
          </p>
        </div>
      ) : (
            ...
            <div key={s.id} className={`bg-[#0f1218] border rounded-xl p-4 transition-colors ${quotaReached || blocked ? 'border-rose-500/50' : 'border-[#1a1f2e] hover:border-violet-500/30'}`}>
              <div className="flex items-start gap-2">
                ...
                <div className="min-w-0">
                  ...
                  {(quotaReached || blocked) && <p className="text-xs text-rose-400 mt-1">{t('commerce.services.unavailable')}</p>}
                  {s.displayProtocol && (
      ...
      {!loading && services.length > 0 && (
        <p className="text-xs text-gray-500">
          {t('commerce.services.count', { count: formatNumber(services.length) })}
        </p>
      )}
    </div>
  );
