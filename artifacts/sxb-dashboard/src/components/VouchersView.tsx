...
export default function VouchersView({ currentUserRole, permissions }: VouchersViewProps) {
  ...
  const loadVouchers = async () => {
    ...
    try {
      ...
      toast.error(err instanceof Error ? err.message : "Chargement des vouchers impossible");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleCreate = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!canCreate) return;
    if (showsOwner && !resellerId) { toast.error("Sélectionnez un revendeur"); return; }
    const diffMs = new Date(`${expiration}T23:59:59`).getTime() - Date.now();
    if (!Number.isFinite(diffMs) || diffMs <= 0) { toast.error("Choisissez une limite d'activation future"); return; }
    setBusy(true);
    try {
      await createVoucher({
      ...
      toast.error(err instanceof Error ? err.message : "Erreur lors de la création du voucher");
    } finally {
      setBusy(false);
    }
  };
  ...
  const handleActivateVoucher = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!canRedeem) return;
    if (!activationInput || !clientId) { toast.error("Choisissez le client et saisissez le code"); return; }
  ...
  const handleRevoke = async (voucher: Voucher) => {
    if (!canRevoke || !window.confirm("Révoquer ce voucher non utilisé et libérer son volume ?")) return;
    setBusy(true);
    try {
                ...
                <table className="w-full text-left border-collapse">
                  <thead>
                    <tr className="border-b border-gray-800/80 bg-gray-900/40 text-xs font-semibold text-gray-400 uppercase tracking-wider">
                      ...
                      <th className="py-3 px-4">Volume réservé</th>
                      {showsOwner && <th className="py-3 px-4">Revendeur</th>}
                  ...
                  <tbody className="divide-y divide-gray-900 text-sm">
                    {filtered.map((v) => {
                      ...
                      const label = { active: "Disponible", used: "Utilisé", expired: "Expiré", revoked: "Révoqué" }[status];
                      return (
                        <tr key={v.id} className="hover:bg-gray-900/20 transition-colors">
                          ...
                          {showsOwner && <td className="py-4 px-4 text-gray-400">{v.reseller?.name || "Historique non attribué"}</td>}
                          <td className="py-4 px-4 text-gray-400">{v.redeemedClient?.name || "Non utilisé"}</td>
                          <td className="py-4 px-4 text-xs text-gray-500 font-mono">
                            {v.expiresAt ? new Date(v.expiresAt).toLocaleDateString() : "Sans échéance historique"}
                            <span className="block mt-1">{v.durationDays ?? 0} jours crédités</span>
                          </td>
                          ...
                          <td className="py-4 px-4">
                            {status === "active" && hasPermission("vouchers.revoke") && (
                              <button onClick={() => handleRevoke(v)} disabled={!canRevoke} title="Révoquer le voucher"
        ...
        <div className="p-6 rounded-xl border border-gray-800/80 bg-gray-950/40 flex flex-col justify-between space-y-4">
          <div>
            ...
            <p className="text-xs text-gray-400 leading-relaxed">
              Choisissez explicitement le client bénéficiaire du même revendeur, puis saisissez son code. Pour un client déjà abonné, modifiez son forfait depuis la section Forfaits.
            </p>
          </div>

          <form onSubmit={handleActivateVoucher} className="space-y-4">
            <div>
              <label className="block text-xs font-semibold text-gray-400 mb-1" htmlFor="voucher-client">Client bénéficiaire</label>
            ...
            <button
              ...
              title={canRedeem ? undefined : "Indisponible : agrément ou permission"}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/70 backdrop-blur-sm">
          <div className="w-full max-w-md p-6 bg-gray-950 border border-gray-800 rounded-xl shadow-2xl relative">
            ...
            <form onSubmit={handleCreate} className="space-y-4">
              {showsOwner && <div>
                <label className="block text-xs font-semibold text-gray-400 mb-1.5" htmlFor="voucher-reseller">Revendeur propriétaire</label>
              ...
              <div>
                <label className="block text-xs font-semibold text-gray-400 mb-1.5" htmlFor="voucher-duration">Durée créditée au client (jours)</label>
