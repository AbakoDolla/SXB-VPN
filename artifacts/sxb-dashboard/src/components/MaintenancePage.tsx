    ...
    <div className="min-h-screen bg-[#07090e] flex flex-col items-center justify-center px-4">
      <div className="w-full max-w-md text-center">
        ...
        <div className="w-16 h-16 rounded-2xl bg-gradient-to-br from-cyan-500 to-blue-600 mx-auto mb-6 flex items-center justify-center shadow-xl shadow-cyan-500/20">
          <img src="/assets/images/logo_sxb_2026.png" alt={t("operations.maintenance.logo")} className="w-11 h-11 object-contain" />
        </div>
        ...
        <div className="bg-[#0a0d14] border border-[#1a1f2e] rounded-2xl p-8 shadow-2xl">
          ...
          <span className="inline-block px-3 py-1 rounded-full bg-amber-500/10 border border-amber-500/25 text-amber-400 text-[10px] font-bold tracking-widest uppercase mb-4">
            {t("operations.maintenance.active")}</span>
          <h1 className="text-xl font-bold text-white mb-2">{t("operations.maintenance.title")}</h1>
          <p className="text-sm text-gray-500 leading-relaxed">
            {t("operations.maintenance.unavailable")}<br />
            {t("operations.maintenance.retryLater")}</p>
          ...
          >
            ...
            {t("operations.maintenance.ownerAccess")}<ChevronRight className={`w-3 h-3 transition-transform ${showOwnerAccess ? 'rotate-90' : ''}`} />
          </button>
          ...
          {showOwnerAccess && (
            <div className="mt-4 p-3 rounded-lg bg-[#07090e] border border-[#1a1f2e]">
              <p className="text-[11px] text-gray-500 mb-2 flex items-center gap-1.5">
                ...
                {t("operations.maintenance.loginAvailable")}</p>
              <button
              ...
              >
                {t("operations.maintenance.login")}</button>
            </div>
          )}
        </div>

        <p className="text-center text-xs text-gray-600 mt-5">{t("operations.maintenance.controlPanel")}</p>
      </div>
    </div>
  );
}
