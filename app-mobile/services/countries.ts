/**
 * countries.ts — Liste des pays proposée dans l'écran « ESSAI GRATUIT ».
 *
 * La localisation est SAISIE : l'utilisateur choisit son pays dans une liste
 * fermée. L'application ne géolocalise JAMAIS l'appareil et n'interroge aucun
 * service d'adresses IP ; la seule aide apportée est le pays déclaré par les
 * réglages du système, et il reste modifiable.
 *
 * Copie conforme de « server/services/countries.ts » : les paquets sont
 * indépendants et ne partagent aucun module. Un test de parité échoue si l'une
 * des copies dérive.
 */
export type CountryLanguage = "fr" | "en";
...
/** « cm » → « CM ». Toute autre forme devient vide et sera refusée. */
...
/** Nom traduit, ou null : l'écran affiche alors son texte d'invite. */
export function countryName(raw: unknown, language: CountryLanguage): string | null {
  const country = BY_CODE.get(normalizeCountryCode(raw));
  return country ? country[language] : null;
}

/**
 * Drapeau en symboles indicateurs régionaux.
 * Aucune image à embarquer, et le rendu se dégrade proprement sur les systèmes
 * qui ne dessinent pas les drapeaux.
 */
export function countryFlag(raw: unknown): string {
  const code = normalizeCountryCode(raw);
  if (!code) return "";
  return String.fromCodePoint(...[...code].map((letter) => 0x1f1e6 + letter.charCodeAt(0) - 65));
}

/** Liste triée selon la langue affichée, pour un sélecteur lisible. */
export function sortedCountries(language: CountryLanguage): Country[] {
  return [...COUNTRIES].sort((a, b) => a[language].localeCompare(b[language], language));
}
