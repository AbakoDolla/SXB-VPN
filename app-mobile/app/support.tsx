...
export default function SupportScreen() {
  ...
  return <LinearGradient colors={colors.gradients.bg as [string, string, string]} style={styles.container}><ScrollView contentContainerStyle={[styles.content, { paddingHorizontal: responsive.screenPadding, gap: responsive.gap, paddingTop: insets.top + spacing.lg, paddingBottom: insets.bottom + layout.tabBarClearance, maxWidth: responsive.contentMaxWidth, width: "100%", alignSelf: "center" }]} showsVerticalScrollIndicator={false} refreshControl={<RefreshControl refreshing={refreshing} onRefresh={() => void loadTickets(true)} tintColor={colors.primary} />}>
    ...
    <SectionHeader icon="create-outline" title={t("contact_us")} colors={colors} tone={colors.accents.indigo} /><View style={[styles.card, { backgroundColor: colors.bgCard, borderColor: colors.border }]}><TextInput ref={subjectRef} style={[styles.input, { color: colors.textPrimary, backgroundColor: colors.bgInput, borderColor: colors.border }]} placeholder={t("ticket_subject_placeholder")} placeholderTextColor={colors.textMuted} value={subject} onChangeText={setSubject} maxLength={200} /><TextInput style={[styles.input, styles.textarea, { color: colors.textPrimary, backgroundColor: colors.bgInput, borderColor: colors.border }]} placeholder={t("ticket_message_placeholder")} placeholderTextColor={colors.textMuted} value={message} onChangeText={setMessage} multiline numberOfLines={4} maxLength={5000} textAlignVertical="top" /><Pressable onPress={handleSend} disabled={sending || !subject.trim() || !message.trim()} style={({ pressed }) => [styles.sendButton, { backgroundColor: sent ? colors.connected : colors.primary }, (!subject.trim() || !message.trim() || sending) && styles.disabled, pressed && styles.pressed]}>{sending ? <ActivityIndicator size="small" color={colors.primaryForeground} /> : <><Ionicons name={sent ? "checkmark" : "send"} size={16} color={colors.primaryForeground} /><Text style={[styles.sendText, { color: colors.primaryForeground }]}>{sent ? t("sent") : t("send")}</Text></>}</Pressable>{sent && <Text style={[styles.successText, { color: colors.connected }]}>{t("ticket_success")}</Text>}</View>
    <SectionHeader icon="ticket-outline" title={t("my_tickets")} colors={colors} tone={colors.accents.violet} trailing={loadingTickets ? <ActivityIndicator size="small" color={colors.primary} /> : undefined} />{!loadingTickets && tickets.length === 0 ? <View style={[styles.emptyCard, { backgroundColor: colors.bgCard, borderColor: colors.border }]}><Ionicons name="file-tray-outline" size={28} color={colors.textMuted} /><Text style={[styles.emptyText, { color: colors.textMuted }]}>{t("ticket_empty")}</Text></View> : tickets.map((ticket) => { const color = statusColor(ticket.status); return <View key={ticket.id} style={[styles.ticketCard, { backgroundColor: colors.bgCard, borderColor: colors.border }]}><View style={styles.ticketTop}><Text style={[styles.ticketTitle, { color: colors.textPrimary }]} numberOfLines={1}>{ticket.title}</Text><View style={[styles.statusBadge, { borderColor: color + "70", backgroundColor: color + "16" }]}><Text style={[styles.statusText, { color }]}>{t(statusKey[ticket.status])}</Text></View></View>{ticket.description && <Text style={[styles.ticketMessage, { color: colors.textSecondary }]} numberOfLines={2}>{ticket.description}</Text>}<Text style={[styles.ticketDate, { color: colors.textMuted }]}>{t("ticket_updated_at")} · {new Date(ticket.updatedAt).toLocaleDateString()}</Text></View>; })}
    <SectionHeader icon="help-circle-outline" title={t("faq")} colors={colors} tone={colors.accents.ambre} /><View style={[styles.card, { backgroundColor: colors.bgCard, borderColor: colors.border, paddingHorizontal: 14 }]}>{faq.map((item, index) => <View key={item.q}><FaqItem item={item} />{index < faq.length - 1 && <View style={[styles.divider, { backgroundColor: colors.border }]} />}</View>)}</View>
  </ScrollView></LinearGradient>;
}

function SectionHeader({ icon, title, colors, tone, trailing }: { icon: string; title: string; colors: ReturnType<typeof import("@/hooks/useColors").useColors>; tone?: string; trailing?: React.ReactNode }) { const teinte = tone ?? colors.accents.cyan; return <View style={styles.sectionHeader}><View style={[styles.sectionIcon, { backgroundColor: teinte + "1F", borderColor: teinte + "3D" }]}><Ionicons name={icon as any} size={15} color={teinte} /></View><Text style={[styles.sectionTitle, { color: colors.textPrimary }]}>{title}</Text>{trailing}</View>; }

const styles = StyleSheet.create({ container: { flex: 1 }, content: { paddingHorizontal: 20, gap: spacing.md }, hero: { alignItems: "center", gap: 9, borderRadius: radius.xl, borderWidth: 1, padding: 23, marginBottom: 8 }, heroIcon: { width: 78, height: 78, borderRadius: radius.xl, borderWidth: 1, alignItems: "center", justifyContent: "center", marginBottom: 3 }, eyebrow: { fontSize: 10, fontFamily: "Inter_700Bold", letterSpacing: 1.5 }, heroTitle: { fontSize: 22, fontFamily: "Inter_700Bold" }, heroSub: { fontSize: 13, lineHeight: 19, fontFamily: "Inter_400Regular", textAlign: "center" }, heroButton: { minHeight: 48, flexDirection: "row", alignItems: "center", gap: spacing.sm, paddingHorizontal: 18, borderRadius: radius.md, marginTop: 3 }, heroButtonText: { fontSize: 13, fontFamily: "Inter_700Bold" }, errorCard: { flexDirection: "row", alignItems: "center", gap: 9, padding: 13, borderWidth: 1, borderRadius: radius.md }, errorText: { flex: 1, fontSize: 12, lineHeight: 18, fontFamily: "Inter_400Regular" }, sectionHeader: { flexDirection: "row", alignItems: "center", gap: spacing.sm, paddingLeft: 4, paddingTop: 8, marginBottom: 3 }, sectionIcon: { width: 28, height: 28, borderRadius: radius.sm, borderWidth: 1, alignItems: "center", justifyContent: "center" }, sectionTitle: { flex: 1, fontSize: 14, fontFamily: "Inter_700Bold" }, card: { borderRadius: radius.lg, borderWidth: 1, padding: 14, gap: spacing.md }, input: { minHeight: 48, borderWidth: 1, borderRadius: radius.md, paddingHorizontal: 13, paddingVertical: 11, fontSize: 13, fontFamily: "Inter_400Regular" }, textarea: { minHeight: 105, paddingTop: 13 }, sendButton: { minHeight: 48, flexDirection: "row", alignItems: "center", justifyContent: "center", gap: spacing.sm, borderRadius: radius.md }, sendText: { fontSize: 13, fontFamily: "Inter_700Bold" }, successText: { fontSize: 12, textAlign: "center", fontFamily: "Inter_500Medium" }, disabled: { opacity: 0.48 }, ticketCard: { borderRadius: radius.lg, borderWidth: 1, padding: 14, gap: spacing.sm, marginBottom: 9 }, ticketTop: { flexDirection: "row", alignItems: "center", gap: spacing.sm, flexWrap: "wrap" }, ticketTitle: { flex: 1, minWidth: 150, fontSize: 14, fontFamily: "Inter_600SemiBold" }, statusBadge: { borderWidth: 1, borderRadius: 99, paddingHorizontal: 8, paddingVertical: 4 }, statusText: { fontSize: 10, fontFamily: "Inter_600SemiBold" }, ticketMessage: { fontSize: 12, lineHeight: 18, fontFamily: "Inter_400Regular" }, ticketDate: { fontSize: 10, fontFamily: "Inter_400Regular" }, emptyCard: { minHeight: 90, alignItems: "center", justifyContent: "center", gap: spacing.sm, borderRadius: radius.lg, borderWidth: 1 }, emptyText: { fontSize: 12, fontFamily: "Inter_400Regular" }, faqItem: { minHeight: 48, paddingVertical: 14, gap: spacing.sm }, faqQuestion: { flexDirection: "row", alignItems: "center", gap: spacing.sm }, faqQuestionText: { flex: 1, fontSize: 13, fontFamily: "Inter_600SemiBold" }, faqAnswer: { fontSize: 13, lineHeight: 20, fontFamily: "Inter_400Regular", paddingLeft: 26 }, divider: { height: 1 }, pressed: { opacity: 0.7, transform: [{ scale: 0.99 }] },
});
