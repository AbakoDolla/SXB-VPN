...
function NotifRow({ item, onMarkRead }: { item: Notification; onMarkRead: (id: string) => void }) {
  ...
  return (
    <Pressable onPress={() => !item.isRead && onMarkRead(item.id)} style={({ pressed }) => [styles.notifCard, !item.isRead && { borderColor: color + "55", backgroundColor: color + "10" }, pressed && styles.pressed]} accessibilityRole="button">
      ...
      <View style={styles.copy}><Text style={[styles.notifTitle, !item.isRead && { color: colors.textPrimary }]}>{item.title}</Text><Text style={styles.notifMsg} numberOfLines={3}>{item.message}</Text><Text style={styles.notifTime}>{timeAgo}</Text>{item.appUpdate && item.downloadUrl && <Pressable disabled={downloading} onPress={async () => { onMarkRead(item.id); setDownloading(true); try { await downloadAndInstallAppUpdate({ versionCode: item.versionCode || 0, versionName: item.versionName || '', apkUrl: item.downloadUrl!, notes: item.notes, minSupportedCode: item.minSupportedCode, forceUpdate: item.forceUpdate }, ); } catch { Alert.alert(t('update_download_error')); } finally { setDownloading(false); } }} style={({ pressed }) => [styles.updateButton, pressed && styles.pressed, downloading && styles.disabled]} accessibilityRole="button"><Ionicons name="download-outline" size={15} color={colors.background} /><Text style={styles.updateButtonText}>{downloading ? t('update_downloading') : t('update_download')}</Text></Pressable>}</View>
    </Pressable>
  );
}
