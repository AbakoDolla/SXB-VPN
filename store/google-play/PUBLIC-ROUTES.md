...
Le parent doit importer le routeur par défaut depuis `./server/routes/public-privacy` et ajouter `app.use("/api/public", publicPrivacyRouter)` dans `server.ts`, **après Helmet mais avant les parseurs globaux `express.json()` / `express.urlencoded()`, les gardes de maintenance et le middleware SPA/static**. Le namespace `/api` est déjà proxifié par Nginx, contrairement aux chemins racine servis par la SPA : aucune modification Nginx n'est nécessaire pour ces URL canoniques. Le parseur du formulaire contrôle lui-même le type et la taille. Ne pas protéger ce routeur par `requireAuth`. Le parent est propriétaire de cette modification ; ce dossier ne touche pas `server.ts`.

Un montage racine optionnel `app.use(publicPrivacyRouter)` peut conserver `/privacy` et `/data-deletion` comme alias si l'hébergement les transmet à Express. **Réutiliser exactement la même instance de routeur** aux deux montages, sans appeler deux fois `createPublicPrivacyRouter`, pour partager le budget de débit. Le cookie CSRF conserve `Path=/`, sa signature et son origine sont communes aux deux chemins. Tous les liens, changements de langue, retours d'erreur et actions de formulaire dirigent vers `/api/public`, même quand la page est ouverte depuis un alias. Les alias ne sont pas les URL à fournir à Play.
...
|---|---|
| `GET /api/public/privacy?lang=fr` ou `en` | HTML autonome, sans auth ni DB requise, pas de script ni ressource tiers. Contenu, coordonnées si configurées, lien vers le formulaire. |
| `GET /api/public/data-deletion?lang=fr` ou `en` | HTML autonome avec étapes, périmètre et formulaire. Sans application installée. Cookie temporaire de protection CSRF. |
| `POST /api/public/data-deletion?lang=fr` ou `en` | Seulement `application/x-www-form-urlencoded`. Crée un `SupportTicket`, jamais une suppression automatique. |

Sans paramètre reconnu, `Accept-Language` négocie FR/EN, puis français par défaut. Les liens mobiles peuvent garder les URL canoniques `https://vpnsxb.afrihall.com/api/public/privacy` et `https://vpnsxb.afrihall.com/api/public/data-deletion` sans paramètre ou fournir la langue. Les pages sont lisibles pendant la maintenance si l'ordre de montage ci-dessus est respecté. Le formulaire a toujours besoin de la base : une base hors service donne une erreur explicite, pas un fallback mémoire.
...
Commande ciblée : `node --test scripts\tests\public-privacy-http.test.mjs scripts\tests\google-play-dossier.test.mjs`. Le test compile les vrais routeurs et l'auth, utilise un store fixture isolé à forme Prisma et n'accède à aucune production. Il monte réellement le routeur sous `/api/public` avant maintenance et couvre accès public, FR/EN, persistance, rôle staff/non-staff, XSS, CSRF/origine/expiration, limites, non-énumération et échec DB. Un cas avec alias racine réutilise la même instance et vérifie CSRF partagé, liens canoniques et impossibilité d'obtenir un second budget en alternant les chemins. Une simulation fixture n'est pas un essai de disponibilité de l'URL déployée.
