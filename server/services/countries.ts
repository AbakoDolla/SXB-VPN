/**
 * countries.ts — Table ISO 3166-1 alpha-2 de référence du serveur.
 *
 * POURQUOI UNE LISTE FERMÉE : le propriétaire veut savoir « d'où viennent nos
 * clients ». Un champ pays en texte libre produirait « Cameroun », « cameroun »,
 * « CMR » et « Cameroon » dans quatre lignes différentes, donc aucune
 * statistique exploitable. La saisie est donc contrainte à un code ISO à deux
 * lettres, validé ici et nulle part ailleurs.
 *
 * ATTENTION : la localisation est SAISIE par l'utilisateur, jamais mesurée.
 * Aucun appel à un service de géolocalisation ni à une base d'adresses IP
 * n'existe dans ce chemin, et il ne doit pas en exister : le pays est une
 * déclaration, pas une observation.
 *
 * Cette table est répliquée à l'identique dans « app-mobile/services/countries.ts »
 * et « artifacts/sxb-dashboard/src/lib/countries.ts » — les trois paquets sont
 * indépendants et ne partagent aucun module. Un test de parité
 * (server/tests/free-trial.test.ts) échoue si l'une des copies dérive.
 */

/** Un pays : code ISO 3166-1 alpha-2 et son nom dans les deux langues. */
...
export const PAYS_ISO: readonly PaysIso[] = [
...
/** Index code → pays, pour une validation en temps constant. */
const PAR_CODE = new Map(PAYS_ISO.map((pays) => [pays.code, pays]));

/** Ensemble des codes acceptés — la liste fermée exigée par la validation. */
export const CODES_PAYS: readonly string[] = PAYS_ISO.map((pays) => pays.code);

/**
 * Normalise une saisie : « cm », « Cm », «  CM  » → « CM ».
 * Toute valeur qui n'est pas exactement deux lettres devient une chaîne vide,
 * ce qui la fait échouer à la validation plutôt que d'être « réparée ».
 */
export function normaliserCodePays(brut: unknown): string {
  if (typeof brut !== "string") return "";
  const propre = brut.trim().toUpperCase();
  return /^[A-Z]{2}$/.test(propre) ? propre : "";
}

/** Vrai si le code appartient à la liste fermée. */
export function estCodePaysValide(brut: unknown): boolean {
  const code = normaliserCodePays(brut);
  return code !== "" && PAR_CODE.has(code);
}

/** Nom traduit d'un pays, ou null si le code est inconnu. */
export function nomPays(brut: unknown, langue: "fr" | "en" = "fr"): string | null {
  const pays = PAR_CODE.get(normaliserCodePays(brut));
  return pays ? pays[langue] : null;
}
