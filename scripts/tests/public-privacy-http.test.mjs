...
async function fixture(t, settings = {}) {
  ...
  app.use(api.createPublicPrivacyRouter({ now: () => clock, settings: api.readPrivacySettings(settings) }));
  ...
  async function form(lang = "en") {
    const response = await get(`/data-deletion?lang=${lang}`);
  ...
  async function post(form, fields = {}, options = {}) {
    ...
    return fetch(`${base}/data-deletion?lang=${fields.lang || "en"}`, {
      method: "POST",
      headers: {
...
test("privacy and deletion are public, localized and available ahead of maintenance", async t => {
  ...
  for (const lang of ["fr", "en"]) {
    for (const route of ["/privacy", "/data-deletion"]) {
      const response = await f.get(`${route}?lang=${lang}`);
  ...
  assert.match(await (await f.get("/privacy", { "Accept-Language": "en-GB,en;q=0.9" })).text(), /<html lang="en">/);
...
test("HTML injection from settings and request data is never rendered as markup", async t => {
  ...
  const html = await (await f.get("/privacy")).text();
...
test("IP limits apply even to invalid attempts; public GET remains readable", async t => {
  ...
  assert.equal((await f.get("/privacy")).status, 200);
  assert.equal((await f.get("/data-deletion")).status, 200);
  assert.equal(f.tickets.length, 0);
});
